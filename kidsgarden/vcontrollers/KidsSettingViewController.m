...
#import "KidsTrafficViewController.h"
#import "KidsAboutViewController.h"
#import "KidsNavigationController.h"

@interface KidsSettingViewController ()
@property(nonatomic,strong)UITableView *tableView;
@end

@implementation KidsSettingViewController
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if(indexPath.section==0&&indexPath.row==0){
        KidsClassListViewController *class_vc=[[KidsClassListViewController alloc]init];
        UINavigationController  *nav_vc = [[KidsNavigationController alloc] initWithRootViewController:class_vc];
        [self presentViewController:nav_vc animated:YES completion:nil];
    }else if(indexPath.section==1&&indexPath.row==0){        
        KidsTrafficViewController *traffic_vc=[[KidsTrafficViewController alloc]init];
        UINavigationController  *nav_vc = [[KidsNavigationController alloc] initWithRootViewController:traffic_vc];
        [self presentViewController:nav_vc animated:YES completion:nil];
    }else if(indexPath.section==2&&indexPath.row==0){
        KidsAboutViewController *about_vc=[[KidsAboutViewController alloc]init];
        UINavigationController  *nav_vc = [[KidsNavigationController alloc] initWithRootViewController:about_vc];
        [self presentViewController:nav_vc animated:YES completion:nil];
    }
